...
export default memo(function PinModal({ profileName, expectedPin, onSuccess, onCancel }) {
  ...
  const triggerKeyHaptic = (val) => {
    if (typeof navigator !== 'undefined' && 'vibrate' in navigator) {
      try { navigator.vibrate(val); } catch {}
    }
  };
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[160] bg-[#09090B]/95 backdrop-blur-2xl flex items-center justify-center p-5 animate-fade-in select-none">
      <div className="w-full max-w-sm relative">
        <div className={`relative bg-[#0A0A0C] border border-white/[0.08] rounded-[2.5rem] p-7 text-center shadow-2xl ${shake ? 'animate-shake' : ''}`}>
          ...
          <button
            onClick={onCancel}
            className="p-2 rounded-full bg-white/[0.03] border border-white/[0.06] text-zinc-400 hover:text-white active:scale-95 transition-all absolute top-5 right-5"
          ...
          <h2 className="text-xl font-black text-white tracking-tight font-sans">{profileName}</h2>
          <p className="text-xs text-zinc-400 mt-1 mb-6 font-mono">
          ...
          {error && (
            <div className="flex items-center justify-center gap-1.5 text-rose-400 text-xs font-semibold mb-4 animate-fade-in font-mono">
              <ShieldAlert size={14} />
              <span>{error}</span>
            </div>
          )}
            ...
            {DIGITS.map((item) => {
              if (item === 'clear') {
                return (
                  <button
                    ...
                    className="h-12 bg-white/[0.02] border border-white/[0.05] rounded-2xl text-zinc-400 text-[10px] font-mono font-bold uppercase tracking-wider hover:text-white active:scale-95 disabled:opacity-20"
                  >
                    Borrar
                  </button>
                );
              ...
              return (
                <button
                  ...
                  disabled={lockedUntil}
                  className={`h-12 rounded-2xl font-black text-xl font-mono transition-all active:scale-90 flex items-center justify-center ${
                    pressedKey === item
