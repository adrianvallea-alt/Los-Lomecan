// src/components/gym/HistoryView.jsx
...
export default function HistoryView({ activeProfile, onBack }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 animate-fade-in safe-top safe-bottom bg-[#09090B] select-none">
      {/* Header */}
        ...
        <div className="px-5 mb-4">
          <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-medium text-zinc-400 font-mono">Volumen (kg) últimos 7 días</span>
            </div>
            {volumes.some(v => v > 0) ? (
              ...
              <p className="text-xs text-zinc-500 font-mono text-center py-2">No hay registros de peso</p>
            )}
          </div>
        </div>
      )}
      ...
      {/* Lista de sesiones */}
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-safe no-scrollbar">
        {loading ? (
          <div className="flex items-center justify-center py-10 text-zinc-500 text-sm font-mono">Cargando...</div>
        ) : filtered.length === 0 ? (
          ...
          <div className="space-y-3 pb-8">
            {filtered.map((session, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-white/[0.02] border border-white/[0.05] rounded-2xl p-4 transition-all hover:border-white/[0.08]"
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-right font-mono">
                      <p className="text-xs text-[#D4FF00] font-bold">Día {session.dayIndex + 1}</p>
