...
export default memo(function DaySelector({ routine, onSelectDay, onBack, completedDays }) {
  const days = routine?.trainingDays || [];
  const completedSet = completedDays?.[routine?.id] || new Set();

  return (
    <div className="flex flex-col h-full animate-fade-in safe-top safe-bottom bg-[#09090B] select-none">
      {/* Header */}
      <div className="flex items-center px-5 pt-5 pb-4">
        <button
          onClick={onBack}
          className="p-2.5 rounded-full bg-white/[0.03] border border-white/[0.08] text-zinc-400 hover:text-white hover:border-[#D4FF00]/40 active:scale-95 transition-all mr-3"
          aria-label="Volver"
        >
          <ArrowLeft size={20} />
        </button>
      ...
      {/* Lista de días */}
      <div className="flex-1 overflow-y-auto px-5 pb-safe no-scrollbar">
        {days.length > 0 ? (
            ...
            {days.map((day, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {isCompleted ? (
                    <span className="text-[10px] text-[#D4FF00] font-medium bg-[#D4FF00]/10 border border-[#D4FF00]/30 px-2.5 py-1 rounded-full flex items-center gap-1 font-mono">
                      <Zap size={10} />
                      Completado
                    </span>
                  ) : (
          ...
          <div className="text-center text-zinc-500 text-sm py-16 font-mono">
